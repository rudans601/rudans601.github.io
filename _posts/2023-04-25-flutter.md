...
부모의 크기에 비례해서 크기를 정해주는 위젯. 그래서 부모의 크기를 꼭 지정해야한다
...
체크박스, 스위치들은 뒤에 .adaptive를 붙이면 운영체제에 맞게 디자인이 변경된다
